...
class HomePage extends Component {
  ...
  // Add this new method to handle showing results
  handleShowResults = (diseaseList) => {
    ...
    switch (this.state.current_page) {
      ...
      case "Disease":
        if (this.state.showResults) {
          // If we're already showing results, reset everything
          return this.setState({
            tab_progress: 25,
            current_page: "Home",
            button_is_disabled: true, // Next button disabled if not agreed to terms
  ...
  showPage = (e) => {
    const { current_page, home_button_checked, age, male, female, showResults } = this.state;
    
    // If we're showing results, return the Results component
    if (showResults && current_page === "Disease") {
      return (
        <Results 
          conditions={this.state.conditions}
          triage={null} // You can add triage data if available
          onRestart={() => this.setState({ showResults: false })}
        />
      );
    }
      ...
      case "Disease":
        return (
          <Disease 
            ...
            age={this.state.age}
            onShowResults={() => this.handleShowResults(this.state.disease_possibility)}
          />
        );
    }
  };
