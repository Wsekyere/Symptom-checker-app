...
class Disease extends Component {
  ...
  get_current_html = () => {
    ...
    // Sort by possibility in descending order (highest percentage first)
    filtered_list.sort((a, b) => {
      return parseFloat(b.possibility) - parseFloat(a.possibility);
    });
    ...
    return filtered_list.length !== 0 ? (
      <div className="grid-row width-full DiseaseComponent">
        ...
        <div className="col-12 tablet:grid-col-12 text-center margin-top-2">
          <button 
            className="usa-button usa-button--big medication-advice-button"
            onClick={() => this.props.onShowResults()}
          >
            <i className="fas fa-pills mr-2"></i> View Detailed Results & Medication Advice
          </button>
        </div>
      </div>
    ) : (
